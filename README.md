...
My current configuration does not use any external plugin since Vim is already a very powerful editor by itself. If you need to install some plugins, you can do so by adding them to the `~/.vim/pack/plugins/start/` directory: the plugins will be loaded automatically at startup and can be updated with a `git pull` command.
